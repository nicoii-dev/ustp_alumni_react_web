...
export function FileHeader({ fileData, onDelete }) {
  return (
    <>
      <div className="flex">
        <span className="mb-1 text-base text-gray-800 mobile:truncate">
          {fileData?.file?.name}
        </span>
        <div className="grid justify-end content-center w-full">
          <button
            type="button"
            className="hover:bg-red-300 rounded-xl z-50"
            onClick={() => onDelete(fileData?.file)}
          >
            <svg
              width="16"
              height="16"
