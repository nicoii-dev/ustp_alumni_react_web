...
const UpdateAchievement = ({ handleClose }) => {
  const dispatch = useDispatch();
  const queryClient = useQueryClient();
  const { updateAchievements } = achievementsApi;
  const { achievement } = useSelector((store) => store.achievement);
  ...
  const { mutate: update, isLoading: updateIsLoading } = useMutation(
    (payload) => updateAchievements(payload, achievement.id),
    {
      onSuccess: (data) => {
